...
def call_openai(
    ...
    """
    # Import OpenAI bên trong hàm để bài test mock có thể tráo openai.OpenAI
    # bằng một đối tượng giả — nếu import ở đầu file, việc tráo này không
    # còn tác dụng nữa (đây là bài học về late binding).
    from openai import OpenAI

    # Hỗ trợ custom base URL (dùng cho 9Router, NVIDIA NIM, v.v.)
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost_estimate = (word_count / 0.75) / 1000 \
        * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
...
def chat_with_system_prompt(
    ...
    """
    # Import OpenAI bên trong hàm để các bài test mock có thể tráo
    # openai.OpenAI bằng đối tượng giả — nếu import ở đầu file, việc
    # tráo này sẽ vô hiệu.
    from openai import OpenAI
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    """
    # tiktoken cần tải bộ mã hóa từ mạng ở lần chạy đầu, và cũng có thể
    # không nhận diện được model lạ. Bọc try/except để rơi về ước lượng
    # dự phòng (trung bình 1 token ≈ 4 ký tự).
    try:
        import tiktoken
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # .get() với fallback: model không có trong bảng giá — ví dụ khi dùng
    # NVIDIA NIM miễn phí — thì lấy giá gpt-4o làm tham chiếu học tập.
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # So sánh trên bản chữ thường — quit/Quit/QUIT đều thoát.
        ...
        # Chunk cuối có delta.content = None — phải dùng `or ""` để
        # tránh TypeError khi ghép chuỗi. Nếu quên, lệnh `reply += delta`
        # sẽ phát sinh: `TypeError: can only concatenate str (not
        # "NoneType") to str`.
        ...
        for chunk in stream:
            ...
            print(delta, end="", flush=True)
            reply += delta
        print()  # newline cuối câu trả lời

        # Cập nhật history rồi cắt còn 3 lượt cuối (6 message). Toàn bộ
        # history được gửi lại mỗi lượt, nên cắt là đánh đổi có chủ đích
        # giữa trí nhớ và chi phí input.
...
def retry_with_backoff(
    ...
    """
    # Lưu ý: vòng lặp phải chạy `max_retries + 1` lần — bao gồm lần gọi
    # đầu cộng các lần thử lại. Viết `range(max_retries)` là thiếu mất
    # một lượt, và hàm sẽ âm thầm trả về None.
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_exception = exc
            # Không sleep sau lần thử cuối cùng — sẽ thử xong và rơi
            # xuống nhánh raise bên dưới.
            if attempt < max_retries:
                delay = base_delay * (2 ** attempt)
                time.sleep(delay)

    # raise trần (không tham số) giữ nguyên exception gốc, nên người gọi
    # biết chính xác lỗi gì thay vì nhận RuntimeError bọc lại.
...
def run_assistant(
    ...
    while True:
        # Kiểm tra max_turns TRƯỚC khi gọi get_input(). Nếu làm ngược
        # lại thì max_turns=0 vẫn đòi gõ phím, và test sẽ trượt với
        # StopIteration khi side_effect của MagicMock cạn.
        if max_turns is not None and num_turns >= max_turns:
            break

        user_msg = get_input().strip()
        # So sánh trên bản chữ thường — quit/Quit/QUIT/Exit đều thoát.
        if user_msg.lower() in ("quit", "exit"):
            break

        # Persona cố định đứng đầu MỖI lượt gọi API — nằm ngoài vùng
        # bị cắt của history, nên nhân vật không trôi mất khi history
        # bị xén còn 3 lượt.
        messages = (
        ...
        # Bọc lời gọi streaming trong retry_with_backoff để chịu lỗi
        # tạm thời (429, mạng chập chờn). Dùng lambda để trì hoãn việc
        # tạo stream cho đến khi retry thật sự chạy.
        def _stream_once():
        ...
        stream = retry_with_backoff(_stream_once, max_retries=3, base_delay=0.1)

        # Chunk cuối có delta.content = None — dùng `or ""` để tránh
        # TypeError khi ghép chuỗi.
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            reply += delta

        # Cập nhật history rồi cắt còn 3 lượt cuối (6 message).
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]

        # Cộng dồn thống kê. estimate_cost() trả input/output riêng;
        # chỉ cần total_cost cho stats cuối phiên.
